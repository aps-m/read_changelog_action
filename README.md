...
В выбранном блоке changelog все записи вида `(#число)` дополняются ссылкой на
задачу в `https://git.aps-m.com/APS_Soft/<репозиторий>/issues/<число>`. Имя
текущего репозитория берётся из переменной окружения `GITHUB_REPOSITORY`
